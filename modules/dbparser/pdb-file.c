...
pdb_file_detect_version(const gchar *pdbfile, GError **error)
{
 ...
 exit:
  ...
  if (!result)
    {
      g_set_error(error, PDB_ERROR, PDB_ERROR_FAILED, "Error detecting pdbfile version, <patterndb> version attribute not found or <patterndb> is not on its own line");
    }
  return result;
}
